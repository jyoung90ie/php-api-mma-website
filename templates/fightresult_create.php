<?php
/**
 * Responsible for creating new record in FightResults table. It will also handle the updating of associated
 * FightAthletes records.
 */
...
if (isset($frResponse['FightID'])) {
    $userNotification = 'A Fight Result already exists - redirect to relevant page.';
    HelperFunctions::addNotification($userNotification);

    header("Location: ?page=fight-result&id=$fightId&action=update");
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (sizeof($validationErrors) == 0) {
        ...
        // process
        $fightResultEndPoint = API_URL . '/fight-result?apiKey=' . API_KEY;
        $result = sendApiRequest($fightResultEndPoint, 'POST', 204,
            $processedFightResultData);
        ...
        print_r($apiResponses);


        if (empty($apiResponse)) {
            HelperFunctions::addNotification('Fight Result added');
    ...
    <main class="container">
        ...
        <form action="" method="post">
                    ...
                    <select name="ResultTypeID" class="form-select" aria-label="Select fight referee">
                        ...
                        if (isset($resultTypeData['data'])) {
                            foreach ($resultTypeData['data'] as $ref) {
                                $selected = ($_POST['ResultTypeID'] == $ref['ResultTypeID'] ? ' selected' : '');
                ...
                <div class="col-auto">
                    <select name="WinnerAthleteID" class="form-select"
                        ...
                        foreach ($fightAthleteData as $athlete) {
                            $selected = (($_POST['WinnerAthleteID'] ?? '') == $athlete['AthleteID'] ? ' selected' : '');
                    ...
                    <select name="WinRound" class="form-select" aria-label="Select the round the fight ended in">
                        ...
                        if (isset($numOfRounds)) {
                            for ($round = 1; $round <= $numOfRounds; $round++) {
                                $selected = (($_POST['WinRound'] ?? '') == $round ? ' selected' : '');
            ...
            <div class="col-6 col-md-4 col-lg-3">
                <label for="' . $athleteFieldName . '" class="col-form-label">' . $fieldDisplayName . '</label>
            </div>';
                    }
            ...
            <button type="submit" class="btn btn-lg btn-success">Add Result</button>
        </form>
    </main>
